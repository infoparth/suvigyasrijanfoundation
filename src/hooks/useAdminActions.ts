...
export const useAdminActions = () => {
  ...
  const uploadPDF = async (
    ...
    try {
      ...
      const fieldName = type === 'question' ? 'questionPaperURL' : 
                       type === 'instructions' ? 'instructionsURL' : 'resultsURL';
      await updateExamStatus({ [fieldName]: publicUrl });

      if (onProgress) onProgress(100);

      const successMessage = type === 'question' ? 'Question paper' : 
                            type === 'instructions' ? 'Instructions' : 'Results';
